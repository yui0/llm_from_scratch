...
def tokenize_to_memmap(
    ...
    """
    logging.info("第1パス: 総トークン数をカウント中...")
    total_tokens = 0
    buffer = []
    with open(text_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(tqdm(f, desc="トークン数カウント")):
            line = line.strip()
            if not line:
                continue
            buffer.append(line)
    ...
    logging.info(f"総トークン数 (block_size={block_size}でアライン): {total_tokens:,}")

    tokens = np.memmap(output_file, dtype=np.int32, mode="w+", shape=(total_tokens,))

    logging.info("第2パス: トークンをmemmapファイルに書き込み中...")
    buffer = []
    offset = 0
    with open(text_path, "r", encoding="utf-8") as f:
        for i, line in enumerate(tqdm(f, desc="memmap書き込み")):
            line = line.strip()
            if not line:
                continue
            buffer.append(line)
...
if __name__ == "__main__":
    ...
    if RUN_MODE == "PREPROCESS":
        ...
        try:
            ...
            # 出力先を一時的なローカルパスに変更
            tokenize_to_memmap(data_path, tokenizer, output_file=LOCAL_MEMMAP_PATH, block_size=BLOCK_SIZE)
            print(f"--- 3. memmapへのトークナイズが完了しました ---")

            print(f"\n--- 4. 作成したmemmapファイルをGoogle Drive ({MEMMAP_PATH}) にコピーします ---")
            # ローカルで作成したファイルをGoogle Driveにコピー
            shutil.copy(LOCAL_MEMMAP_PATH, MEMMAP_PATH)
            print(f"--- 4. ファイルのコピーが完了しました ---")
