...
import shutil
import time  # 追加: タイムアウト対策のためのスリープ
...
# -----------------------------
# Colab タイムアウト対策: 定期的なアクティビティ出力
# -----------------------------
def keep_alive():
    """Colabのタイムアウトを防ぐための定期出力とJavaScriptハック"""
    # 改善: コンソール出力を削除してうるさくならないように
    # Colab固有のJavaScriptで定期クリックシミュレーション（オプション）
    try:
        ...
        display(Javascript('''
            ...
            setInterval(ClickConnect, 60000)  # 1分ごとにクリック
        '''))
    except:
        pass
...
def forward(params, x: jnp.ndarray, rng_key: jnp.ndarray, embed_size: int = 256, num_heads: int = 4, head_size: int = 64, dropout_rate: float = 0.1, training: bool = True, compute_dtype=jnp.bfloat16) -> jnp.ndarray:  # 改善: パラメータ調整
    ...
    def layer_fn(embeddings, layer_params, rng_key):
        ...
        embeddings = layer_norm(embeddings, layer_params["attn_norm"]["scale"], layer_params["attn_norm"]["bias"])  # 注意: 元コードのtypo修正 (attn_norm)
...
# -----------------------------
# 7. チェックポイント関連 - 改善: expectedパラメータ調整、頻度増加対応
# -----------------------------
...
# -----------------------------
# 8. 学習ループ（memmap + GPU効率化 + 再開対応） - 改善: grad_accum_steps=16, n_iterations=20000, dropout=0.05, タイムアウト対策追加（keep_alive呼び出し、ckpt_every減少）
# -----------------------------
def train_model_memmap(
    ...
    ckpt_every: int = 100,  # 改善: 頻度を増やしてタイムアウト時再開しやすく
):
    ...
    if last is not None:
        try:
            # Fixed: Use correct keyword args
            payload = load_checkpoint(last, expected_embed_size=256, expected_num_heads=4, expected_head_size=64)
    ...
    for i in pbar:
        ...
        # 追加: 長時間ループ時の短いスリープでリソース節約（オプション、必要に応じて調整）
        if i % 1000 == 0:
            time.sleep(1)  # 1秒スリープでColab安定化
...
# -----------------------------
# 8.5 Fine-Tuningループ（SFT用） - 修正: 損失関数をJAX互換のマスクCEに変更（boolean index回避）、データセットスプリットにselect使用、n_iterations減少、val追加、タイムアウト対策
# -----------------------------
def fine_tune_model(
    params,
    tokenizer: Tokenizer,
    ...
    split: str = "train",
    batch_size: int = 16,
    n_iterations: int = 5000,  # 修正: 小さなデータセット用に減少（過学習回避）
    ...
    ckpt_every: int = 100,
    val_ratio: float = 0.1,  # 追加: 検証スプリット
):
    try:
    ...
    logging.info(f"Filtered dataset size: {len(dataset)} examples")
    ...
    def preprocess(example):
        ...
        labels = [-100] * len(prompt_ids) + response_ids  # プロンプトを-100でマスク
        ...
        # 修正: ログを1回だけ（map内で複数呼び出し回避）
        if len(dataset) < 10:  # 小さなデータセットの場合のみログ
    ...
    # 修正: selectを使用してDatasetオブジェクトを保持
    ...
    # 修正: 損失関数 - シフト + マスク付きクロスエントロピー（boolean index回避）
    def ft_loss_fn(p, inputs, labels, key):
        logits = forward(p, inputs, key, training=True, compute_dtype=compute_dtype, dropout_rate=dropout_rate, embed_size=256, num_heads=4)
        shift_logits = logits[:, :-1, :]
        shift_labels = labels[:, 1:]
        ce_loss = optax.softmax_cross_entropy_with_integer_labels(shift_logits, shift_labels)
        mask = (shift_labels != -100)
        masked_loss = ce_loss * mask
        num_valid = jnp.sum(mask)
        return jnp.where(num_valid > 0, jnp.sum(masked_loss) / num_valid, jnp.array(0.0, dtype=jnp.float32))
    ...
    @jax.jit
    def eval_step(p, inputs, labels, key):  # 追加: 評価用（training=False）
        def eval_loss_fn(p, inputs, labels, key):
            logits = forward(p, inputs, key, training=False, compute_dtype=compute_dtype, dropout_rate=0.0, embed_size=256, num_heads=4)
            shift_logits = logits[:, :-1, :]
            shift_labels = labels[:, 1:]
            ce_loss = optax.softmax_cross_entropy_with_integer_labels(shift_logits, shift_labels)
            mask = (shift_labels != -100)
            ...
            num_valid = jnp.sum(mask)
            return jnp.where(num_valid > 0, jnp.sum(masked_loss) / num_valid, jnp.array(0.0, dtype=jnp.float32))
        return eval_loss_fn(p, inputs, labels, key)

    train_losses, val_losses = [], []
    best_val = float("inf")
    patience = 200  # 追加: 早期停止
    patience_counter = 0
    start_iter = 0
    last = latest_checkpoint(ckpt_dir)
    if last:
        try:
            ...
            logging.info(f"Resumed fine-tuning from {last} (step={start_iter-1}, best_val={best_val:.4f})")
        except ValueError as e:
    ...
    for i in pbar:
        ...
        batch_idx = (i * batch_size) % train_size
        end_idx = min(batch_idx + batch_size, train_size)
        batch = train_dataset[batch_idx : end_idx]
        # 修正: 直接使用（既にパッド済み、スタック済みJAX配列）
        inputs = batch["input_ids"]  # shape: (actual_bs, block_size)
        labels = batch["labels"]    # shape: (actual_bs, block_size)
        
        global rng_key
        rng_key, subkey = random.split(rng_key)
        params, opt_state, loss = update_step(params, opt_state, inputs, labels, subkey)
        train_losses.append(float(loss))

        if i % 50 == 0:  # 追加: 検証評価
            val_batch_size = min(batch_size, len(val_dataset))
            val_batch = val_dataset[:val_batch_size]  # 修正: 最小バッチサイズでスライス
            v_inputs = val_batch["input_ids"]
            v_labels = val_batch["labels"]
            rng_key, v_subkey = random.split(rng_key)
            v_loss = float(eval_step(params, v_inputs, v_labels, v_subkey))
            v_perp = perplexity(v_loss)
            val_losses.append(v_loss)
            ...
            if improved:
                best_val = v_loss
                patience_counter = 0
                save_checkpoint(ckpt_dir, i, params, opt_state, best_val)
            else:
                patience_counter += 1
...
# -----------------------------
# 11. メイン実行ブロック - 改善: dataset_configsを更新（mc4削除、oscar追加）、再開確認メッセージ追加
# -----------------------------
if __name__ == "__main__":
    ...
    # 追加: チェックポイント存在確認と再開メッセージ
    pretrain_ckpt = latest_checkpoint(CHECKPOINT_DIR)
    ft_ckpt = latest_checkpoint(FT_CHECKPOINT_DIR)
    if pretrain_ckpt:
        print(f"Pretraining checkpoint found: {pretrain_ckpt} - Will resume from here.")
    if ft_ckpt:
        print(f"Fine-tuning checkpoint found: {ft_ckpt} - Will resume from here.")
