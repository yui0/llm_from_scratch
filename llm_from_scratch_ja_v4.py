...
def prepare_data(dataset_name: str = "wikimedia/wikipedia", config: str = "20231101.ja", split: str = "train", chunk_size: int = 1000, output_path: str = "input.txt"):
    ...
    try:
        if os.path.exists(output_path):
            logging.info(f"Dataset file already exists at {output_path}. Skipping preparation.")
            return output_path
...
def loss_fn(params, x: jnp.ndarray, y: jnp.ndarray, rng_key: jnp.ndarray, **fw_kwargs) -> jnp.ndarray:
    logits = forward(params, x, rng_key, **fw_kwargs)  # Removed explicit training=True
...
if __name__ == "__main__":
    ...
    # memmapへのトークナイズ（ローカルにファイルがなければ作成、Google Driveにもコピー）
    if not os.path.exists(LOCAL_MEMMAP_PATH):
        tokenize_to_memmap(data_path, tokenizer, output_file=LOCAL_MEMMAP_PATH, block_size=BLOCK_SIZE, drive_path=MEMMAP_PATH)
    else:
        logging.info(f"Tokens memmap already exists at {LOCAL_MEMMAP_PATH}. Loading it.")
        tokens = np.memmap(LOCAL_MEMMAP_PATH, dtype=np.int32, mode="r+")
